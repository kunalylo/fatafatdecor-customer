...
export default function AuthScreen() {
  ...
  return (
    <div className="min-h-screen bg-rose-50 fade-in flex flex-col">
      ...
      {/* ── Pink hero ── */}
      <div className="gradient-pink pt-14 pb-16 px-6 text-center relative overflow-hidden">
        {/* Decorative blobs */}
        <div className="absolute top-0 right-0 w-40 h-40 rounded-full bg-white/10 -translate-y-1/2 translate-x-1/2 pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-28 h-28 rounded-full bg-white/10 translate-y-1/2 -translate-x-1/2 pointer-events-none" />

        <div className="w-24 h-24 rounded-full overflow-hidden mx-auto mb-4 border-4 border-white/40 shadow-2xl relative z-10">
          <img src={LOGO_URL} alt="FatafatDecor" className="w-full h-full object-cover" />
        </div>
        <h1 className="text-white text-2xl font-black tracking-tight relative z-10">FatafatDecor</h1>
        <p className="text-white/80 text-sm mt-1 relative z-10">✨ AI-powered decoration at your doorstep</p>
        <div className="flex justify-center gap-4 mt-3 relative z-10">
          {['🎀', '🎊', '🌸', '💐', '🎉'].map((e, i) => (
            <span key={i} className="text-xl opacity-80">{e}</span>
          ))}
        </div>
      </div>

      {/* ── Card overlapping hero ── */}
      <div className="flex-1 flex flex-col items-center px-4 -mt-8 pb-8">
