...
export default function AuthScreen() {
  const { authMode, setAuthMode, loading, authForm, setAuthForm, signupOtpSent, setSignupOtpSent, signupOtpValue, setSignupOtpValue, devOtp, setDevOtp, showToast, handleGoogleAuth, handleAuth, handleSendSignupOtp, handleVerifySignupOtp } = useApp()
  return (
      ...
      <CardContent className="p-6 space-y-4">
        <div className="flex gap-2 mb-4">
          {['login', 'register'].map(m => (
            <button key={m} onClick={() => { setAuthMode(m); setSignupOtpSent(false); setSignupOtpValue(''); setDevOtp('') }}
        ...
        {authMode === 'register' && (
          <>
            ...
            <Input placeholder="Phone Number (10 digits)" type="tel" value={authForm.phone}
              ...
              className="bg-gray-50 border-gray-200 h-12 rounded-xl" />
            <Button onClick={handleSendSignupOtp} disabled={loading} className="w-full h-10 gradient-pink border-0 text-white font-semibold text-sm rounded-xl shadow-pink">
              ...
              <>
                {devOtp && (
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-center">
                    <p className="text-xs text-amber-600 font-medium mb-1">Dev Mode — OTP (add Fast2SMS key for real SMS)</p>
                    <p className="text-2xl font-bold tracking-[0.4em] text-amber-800">{devOtp}</p>
                  </div>
                )}
                <Input placeholder="Enter 6-digit OTP" value={signupOtpValue} onChange={e => setSignupOtpValue(e.target.value.replace(/\D/g, '').slice(0, 6))}
